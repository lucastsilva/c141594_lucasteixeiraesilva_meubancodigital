    ...
    extratoArea("#extrato-stats", 100, '#4DB6AC'); // initialize chart

    // Chart setup
function extratoArea(element, height, color) {
        ...
        jQuery.each(myTableArray, function (index, item) {
            newTableArray.push({
                date: $(item)[0] === "" ? 0 : $(item)[0],
                value: parseInt($(item)[1].replace(/\./g, "").slice(0, -5))
            });
        });

        var jsonString = JSON.stringify(newTableArray);
